...
describe("API Tests", () => {
  describe("POST /users", () => {
    it("debería agregar un usuario válido", async () => {
      ...
      // Aquí no usamos el mock, el código real se ejecutará
  ...
  describe("GET /user/:id", () => {
    it("debería devolver un usuario válido con sus libros", async () => {
      const mockUser = {id: 1, name: 'Juan Pérez', email: 'juan.perez@example.com'}
      const mockBooks = [
        {
    ...
    it("debería devolver un error si el ID no es numérico", async () => {
      ...
      expect(res.body).toEqual({
        success: false,
        message: "The provided user ID (abc) is not valid. IDs must be numeric.",
        status: 400,
      });
    });
